...
class CircuitBuilder extends IRTreeBuilder[Circuit] {
  private val constantSockets = Map(
    true -> new ConstantSocket(true),
    false -> new ConstantSocket(false)
  )
  private val _clocks = MListBuffer.empty[(Int, Socket)]

  // a socket that represents a constant 1 signal
  def trueSocket: Socket = constantSockets(true)

  // a socket that represents a constant 0 signal
  def falseSocket: Socket = constantSockets(false)

  // get the list of clocked inputs in this circuit
  ...
  def newConstNode(value: Boolean): Circuit = {
    return new Circuit(constantSockets(value))
  }
  ...
  def and(circuit1: Circuit, circuit2: Circuit): Circuit = connectToGate(
    AndGate,
    circuit1,
    circuit2
  )
  ...
  def or(circuit1: Circuit, circuit2: Circuit): Circuit = connectToGate(
    OrGate,
    circuit1,
    circuit2
  )
  ...
  def xor(circuit1: Circuit, circuit2: Circuit): Circuit = connectToGate(
    XorGate,
    circuit1,
    circuit2
  )
  ...
  def nand(circuit1: Circuit, circuit2: Circuit): Circuit = connectToGate(
    NandGate,
    circuit1,
    circuit2
  )
  ...
  def nor(circuit1: Circuit, circuit2: Circuit): Circuit = connectToGate(
    NorGate,
    circuit1,
    circuit2
  )
  ...
  def xnor(circuit1: Circuit, circuit2: Circuit): Circuit = connectToGate(
    XnorGate,
    circuit1,
    circuit2
  )
  ...
  private def connectToGate(gate: Gate, circuits: Circuit*): Circuit = {
    ...
    gate match {
      ...
      case AndGate => AndGate(
        circuits(0).outputs(0),
        circuits(1).outputs(0),
        destination
      )
      case OrGate => OrGate(
        circuits(0).outputs(0),
      ...
      case XorGate => XorGate(
        circuits(0).outputs(0),
        circuits(1).outputs(0),
        destination
      )
      case NandGate => NandGate(
        circuits(0).outputs(0),
        circuits(1).outputs(0),
        destination
      )
      case NorGate => NorGate(
        circuits(0).outputs(0),
        circuits(1).outputs(0),
        destination
      )
      case XnorGate => XnorGate(
        circuits(0).outputs(0),
        circuits(1).outputs(0),
        destination
      )
