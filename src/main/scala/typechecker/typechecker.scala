...
object BoolexTypeChecker {
  ...
  final private class BoolexTypeCheckerImpl {
    ...
    def checkModule(
    ...
    ): (Seq[CompileTimeError], Option[CircuitMetaData]) = {
      val duplicateCircuitNameErrors = (for {
        ...
        (!scopes.addSymbol(name, PartialCircuitType(inputs))).optionally(
          MiscError("Duplicate identifier: \'" + name + "\'", Some(pos))
        )
      }).flatten
      val noMainError = (!scopes.containsSymbol("main")).optionalList(
        MiscError("Could not find \'main\' circuit")
      )
      if (duplicateCircuitNameErrors.size + noMainError.size > 0) {
        ...
        if (cyclicErrors.nonEmpty) {
          ...
          val circuitSpecs = (for {
            ...
            typeData <- scopes
              .getSymbolType(name)
              .map(_.asInstanceOf[CircuitType])
          } yield {
          ...
          return (
            ...
            Some(
              CircuitMetaData(
                getTotalDependencyMap(dependencyGraph),
                circuitSpecs
              )
            )
          )
    ...
    def checkCircuitDeclaration(
      declaration: CircuitDeclarationContext
    ): Seq[CompileTimeError] = {
      ...
      val nameErrors = (if (circuitName == "_") {
        ...
        TypeError(
          "\'_\' is an invalid name for a formal parameter",
          Some(formal.pos)
        )
      })
      ...
      val duplicateParameterNamesErrors = for {
        ...
        MiscError(
          "Duplicate identifier: \'" + formal.name + "\'",
          Some(formal.pos)
        )
      }
      ...
      val warnings = if (allErrors.isEmpty) {
        val outputDependencyGraph = getTotalDependencyMap({
          ...
          val localDependencies = for {
            ...
            dependencies = assignment
              .values
              .flatMap(getVariablesInExpression)
              .toSet
        ...
        val allVariablesByName = (
          formals ++:
          assignments.flatMap(_.variables).filter(_.name != "_")
        ).mapBy(_.name)
        val inputDependencyGraph = outputDependencyGraph.invert
        val variablesAffectedByInput = inputDependencyGraph
          .getOrElse("%input%", Nil)
        val variablesAffectingOutput = outputDependencyGraph
          .getOrElse("%output%", Nil)
        val variablesNotAffectedByInput =
          allVariablesByName.keys.toSet -- variablesAffectedByInput
        val variablesNotAffectingOutput =
          allVariablesByName.keys.toSet -- variablesAffectingOutput
        (for (name <- variablesNotAffectedByInput) yield {
          Warning(
            "Variable \'" + name + "\' is not affected by the input to " +
                "circuit \'" + circuitName + "\'",
            allVariablesByName.get(name).map(_.pos)
          )
        }) ++: (for (name <- variablesNotAffectingOutput) yield {
          Warning(
            "Variable \'" + name + "\' does not affect the output of " +
                "circuit \'" + circuitName + "\'",
            allVariablesByName.get(name).map(_.pos)
          )
        }).toList
    ...
    def checkAssignment(
      ...
      val outputs = values.flatMap(value => {
        List.fill(countExpressionOutputs(value))(value)
      })
      val assignmentErrors = (if (variables.size > outputs.size) {
        val variable = variables(outputs.size)
        Some(
          MiscError(
            "Missing value for variable \'" + variable.name + "\'",
            Some(variable.pos)
          )
        )
        ...
        Some(
          MiscError(
            "Missing variable for expression. Be sure to account for all " +
                "expression outputs.",
            Some(value.pos)
          )
        )
      ...
      val duplicateIdentifierErrors = (for {
        ...
        (!scopes.fillPromise(variable.name)).optionally(
          MiscError(
            "Duplicate identifier: \'" + variable.name + "\'",
            Some(variable.pos)
          )
        )
      }).flatten
    ...
    def checkOutStatement(
      outStatement: OutStatementContext
    ): Seq[CompileTimeError] = {
      val currentCircuit = scopes.getOwner
      val outputs = outStatement.outputs
      if (currentCircuit != "_") {
        scopes.completeCircuit(
          currentCircuit,
          outputs.map(countExpressionOutputs).sum
        )
      }
    ...
    def checkExpression(
    ...
    ): Seq[CompileTimeError] = expression match {
      ...
      case AndExpression(exp1, exp2) => {
        checkUnaryExpression(exp1) ++: checkUnaryExpression(exp2)
      }
      ...
      case Variable(name) => if (name == "_") {
        ...
        (for {
          ...
          "\'" + name + "\' is not a true/false value",
          Some(expression.pos)
        )).orElse({
      ...
      case Clock(milliseconds) => if (milliseconds.number.toInt < 100) {
        List(
          MiscError(
            "Clock period must be greater than 100ms",
            Some(milliseconds.pos)
          )
        )
      } else if (milliseconds.number.toInt > 1000000) {
        List(
          MiscError(
            "Clock period must be less than 5,000,000ms",
            Some(milliseconds.pos)
          )
        )
      } else {
      ...
      case CircuitCallContext(name, arguments) => {
        ...
        val typeErrors = if (typeOpt.exists(_.isInstanceOf[CircuitType])) {
          val numFormalParameters = typeOpt
            .map(_.asInstanceOf[CircuitType].inputs)
            .getOrElse(0)
          val numActualParameters = arguments.map(countExpressionOutputs).sum
          (numActualParameters != numFormalParameters).optionalList(
            TypeError(
              "Expected " + numFormalParameters + " parameters for " +
                  name.name + " but " + numActualParameters + " arguments " +
                  "were found",
              Some(name.pos)
            )
          )
    ...
    def checkUnaryExpression(
      expression: ExpressionContext
    ): Seq[CompileTimeError] = {
      val outputs = countExpressionOutputs(expression)
      return (outputs > 1).optionalList({
        TypeError(
          "Expected expression with 1 output but found expression " +
              "with " + outputs,
          Some(expression.pos)
        )
    ...
    def countExpressionOutputs(exp: ExpressionContext): Int = exp match {
      case CircuitCallContext(name, arguments) => scopes
        .getSymbolType(name.name)
        .map(_.asInstanceOf[CircuitType].outputs)
        .getOrElse(0)
      case _ => 1
    }

    def getVariablesInExpression(
      exp: ExpressionContext
    ): Seq[String] = exp match {
      case BufferExpression(exp) => getVariablesInExpression(exp)
      case NotExpression(exp) => getVariablesInExpression(exp)
      case AndExpression(exp1, exp2) => {
        getVariablesInExpression(exp1) ++: getVariablesInExpression(exp2)
      }
      case NandExpression(exp1, exp2) => {
        getVariablesInExpression(exp1) ++: getVariablesInExpression(exp2)
      }
      ...
      case CircuitCallContext(_, arguments) => {
        arguments.flatMap(getVariablesInExpression)
      }
    }

    def getCircuitDependencies(
      declaration: CircuitDeclarationContext
    ): Set[String] = {
      def _getCircuitDependencies(
        context: Context
      ): Set[String] = context match {
        case AssignmentContext(_, values) => {
          values.map(_getCircuitDependencies).flatten.toSet
        }
        case OutStatementContext(outputs) => {
          outputs.map(_getCircuitDependencies).flatten.toSet
        }
        case BufferExpression(exp) => {
          _getCircuitDependencies(exp)
        }
        case NotExpression(exp) => {
          _getCircuitDependencies(exp)
        }
        case AndExpression(exp1, exp2) => {
          _getCircuitDependencies(exp1) ++ _getCircuitDependencies(exp2)
        }
        case NandExpression(exp1, exp2) => {
          _getCircuitDependencies(exp1) ++ _getCircuitDependencies(exp2)
        }
        ...
        case CircuitCallContext(name, arguments) => {
          arguments.map(_getCircuitDependencies).flatten.toSet + name.name
        }
        case _ => Set.empty[String]
      }
      (declaration.output +: declaration.assignments)
        .map(_getCircuitDependencies).flatten.toSet
    }

    def checkCyclicDependencies[A](
      dependencyGraph: Map[A, Set[A]]
    ): Option[MiscError] = {
      val mutableGraph = scala.collection.mutable.Map() ++ dependencyGraph
      def _checkCyclicDependencies(
        node: A,
        parents: List[A]
      ): (Set[A], Option[MiscError]) = {
        val checked = (for {
          ...
          if (parents.contains(dependency)) {
            ...
            val dependencyError = {
              val cycle = if (cyclicParents.contains(node)) {
                cyclicParents.reverse
              } else {
                (node +: cyclicParents).reverse
              }
              MiscError(
                "Circular dependency: " + (node +: cycle).mkString(" -> ")
              )
            }
            return (Set(node), Some(dependencyError))
          } else {
            val (checked, errorOpt) = _checkCyclicDependencies(
              dependency,
              node +: parents
            )
