...
object BoolexParseTreeRewriter {
  ...
  final private class BoolexParseTreeRewriterImpl(
    nonInputCircuits: Set[String]
  ) {
    ...
    def rewriteAssignment(assignment: AssignmentContext): AssignmentContext = {
      val newAssignment = AssignmentContext(
        assignment.variables,
        assignment.values.map(rewriteExpression)
      )
      newAssignment.setPos(assignment.pos)
      return newAssignment
    }

    def rewriteOutStatement(
      outStatement: OutStatementContext
    ): OutStatementContext = {
      val newOutStatement = OutStatementContext(
        outStatement.outputs.map(rewriteExpression)
      )
    ...
    def rewriteExpression(expression: ExpressionContext): ExpressionContext = {
      val newExpression = expression match {
        ...
        case AndExpression(exp1, exp2) => AndExpression(
          rewriteExpression(exp1),
          rewriteExpression(exp2)
        )
        case NandExpression(exp1, exp2) => NandExpression(
          rewriteExpression(exp1),
          rewriteExpression(exp2)
        )
        case XorExpression(exp1, exp2) => XorExpression(
          rewriteExpression(exp1),
          rewriteExpression(exp2)
        )
        case XnorExpression(exp1, exp2) => XnorExpression(
          rewriteExpression(exp1),
          rewriteExpression(exp2)
        )
        case OrExpression(exp1, exp2) => OrExpression(
          rewriteExpression(exp1),
          rewriteExpression(exp2)
        )
        case NorExpression(exp1, exp2) => NorExpression(
          rewriteExpression(exp1),
          rewriteExpression(exp2)
        )
        case Variable(name) if nonInputCircuits.contains(name) => {
          // re-cast variable as circuit call
          val symbol = Symbol(name)
          symbol.setPos(expression.pos)
          CircuitCallContext(symbol, Nil)
        }
        case CircuitCallContext(name, arguments) => CircuitCallContext(
          name,
          arguments.map(rewriteExpression)
        )
        case _ => expression
      }
