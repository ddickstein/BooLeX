...
// Note that this preprocessor drops positional information,
// but all error checking has already happened, so this is ok
object Boolex2VhdlPreprocessor {
  import parser.contexts._
  import typechecker.CircuitMetaData

  // VHDL reserved words and other words reserved for translation
  private val reservedWords = Set(
    ...
    "integer", "is", "label", "library", "linkage", "literal", "loop", "map",
    ...
    "report", "reset", "return", "rol", "ror", "select", "severity", "signal",
  ...
  final private class Boolex2VhdlPreprocessorImpl(
    module: ModuleContext,
    oldMetaData: CircuitMetaData
  ) {
    private val renameCircuit = getNewCircuitNameMap

    // ok b/c will be set before use
    private var renameVariable: Map[String,String] = null

    // ok b/c will be set before use
    private var tempGenerator: Iterator[String] = null
    ...
    val metaData = {
      ...
      val directCircuitClocksMap = (for {
        ...
        clocksInAssignments = circuitContext
          .assignments
          .flatMap(_.values)
          .flatMap(getClocks)
          .toSet
    ...
    def preprocessModule: ModuleContext = {
      return ModuleContext(module.circuits.map(preprocessCircuit))
    }

    private def preprocessCircuit(
      circuit: CircuitDeclarationContext
    ...
    private def preprocessAssignment(
      assignment: AssignmentContext
    ): Seq[AssignmentContext] = {
      val (newAssignments1, newExpressions) = assignment
        .values
        .map(preprocessExpression)
        .flatUnzip
      val replacementMap = MHashMap.empty[String, String].withDefault(identity)
      val newAssignments2 = (for {
        ...
        if (
          expression.isInstanceOf[Variable] &&
          expression.asInstanceOf[Variable].name.startsWith("temp_")
        ) {
          replacementMap += (
            expression.asInstanceOf[Variable].name -> newVariable
          )
          None
        } else {
      ...
      for {
        ...
        variables = newAssignment
          .variables
          .map(sym => Symbol(replacementMap(sym.name)))
        values = newAssignment.values
      } yield AssignmentContext(variables, values)
    }

    private def preprocessOutStatement(
      outStatement: OutStatementContext
    ): (Seq[AssignmentContext], OutStatementContext) = {
      val outGenerator = Iterator.from(1).map(x => "out_%02d".format(x))
      val (newAssignments1, newExpressions) = outStatement
        .outputs
        .map(preprocessExpression)
        .flatUnzip
      ...
      return (
        ...
        OutStatementContext(newOutputs.map(Variable))
      )
    }

    private def preprocessExpression(
      expression: ExpressionContext
    ): (Seq[AssignmentContext], Seq[ExpressionContext]) = expression match {
      case NotExpression(exp) => {
        val (assignments, newExp) = preprocessExpression(exp)
          .mapRight(_.apply(0))
      ...
      case AndExpression(exp1, exp2) => {
        val (assignments1, newExp1) = preprocessExpression(exp1)
          .mapRight(_.apply(0))
        val (assignments2, newExp2) = preprocessExpression(exp2)
          .mapRight(_.apply(0))
      ...
      case CircuitCallContext(
        name: Symbol,
        arguments: Seq[ExpressionContext]
      ) => {
        var (assignments, newExpressions) = arguments
          .map(preprocessExpression)
          .flatUnzip
        ...
        val variables = {
          List.fill(oldMetaData.circuitSpecs(name.name)._2)(tempGenerator.next)
        }
        assignments :+= AssignmentContext(
          variables = variables.map(Symbol),
          values = List(
            CircuitCallContext(
              Symbol(renameCircuit(name.name)),
              newArguments
            )
          )
        )
    ...
    private def getClocks(
      expression: ExpressionContext
    ): Set[Int] = expression match {
    ...
    private def getNewCircuitNameMap: Map[String, String] = {
      ...
      (for {
        ...
        newName = if (
          ...
          reservedWords.contains(baseName)
        ) {
    ...
    private def getNewVariableNameMap(
      circuit: CircuitDeclarationContext
    ): Map[String, String] = {
      val nameGenerator = Iterator.from(1).map(x => "__%02d".format(x))
      val baseCircuitNames = module
        .circuits
        .map(_.name.name.replace("_","").toLowerCase)
        .toSet
      val usedNames = MHashSet.empty[String]
      ...
      (for {
        ...
        baseVarName = baseName
          .applyIf(name => baseCircuitNames.contains(name))("var_" + _)
        newName = if (
          usedNames.contains(baseVarName) ||
          reservedWords.contains(baseVarName)
        ) {
